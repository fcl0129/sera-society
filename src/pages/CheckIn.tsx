...
export default function CheckIn() {
  ...
  const normalizeNfcInput = (rawValue: string) => {
    const value = rawValue.trim();
    if (!value) return value;

    // Raw NFC tag value entered manually
    ...
    try {
      ...
      // Accept links generated by the device pass page: /nfc-pass/<tag>
      const passMatch = url.pathname.match(/\/nfc-pass\/([^/]+)/);
      if (passMatch?.[1]) {
        return decodeURIComponent(passMatch[1]);
      }

      // Fallback: allow ?tag=<nfc_tag>
      const tagParam = url.searchParams.get("tag");
      if (tagParam) return tagParam;
    } catch {
      // Ignore parse errors and use raw input below.
    }
  ...
  const redeemTicket = async (mode: "qr" | "nfc") => {
    ...
    const value = mode === "qr" ? qrCodeInput.trim() : normalizeNfcInput(nfcTagInput);
    if (!value) return;
